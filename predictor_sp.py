...
def main():
    ...
    corpus = Bigram_freq.get_lines("spanish.corpus.txt") 
    
    #here we pass our lexicon (the dictionary to check the autocorrected words, so we can have real words) I added encoding to cover for characters that are not in english language.

    lexicon = set(open("lexicon_sp.txt",encoding="utf-8").read().split())
    
    #the file alpha.txt passes the alphabet in the 3rd position of our argument in the terminal 
    alphabet = open("alpha_sp.txt",encoding="utf-8").read().split()
    
    #we tokenize our text, giving the instruction to follow the spanish stucture
    cleaned_lines = sent_tokenize(corpus,language="spanish")
    ...
    print("Si quieres cerrar el programa, pulsa q")
    ...
    while True:
        ...
        if not message:
            ...
            print(f"El mensaje completo es:"," ".join(message))
 ...
 # we will check if there are not suggestion available for a word we typed.
            ...
            if len(new_word) == 0:
                print(f"No hay sugerencias para esta opción.")
                print(f"Aquí hay 3 opciones aleatorias.")
                ...
                if len(corrector) == 1:
                    print(f"¿Quisiste decir '{corrector[0]}'? s/n?")
                    if input().lower() == "s":
                        word = corrector[0]
                        message.append(word)
                        continue
                ...
                # If we have more options we allow the user to chose
                    ...
                    print("¿Quisiste decir alguna de las siguientes opciones?:")
            ...
            print(f"o escribe otra palabra:")
        ...
        if word not in lexicon:
            ...
            if len(fixer) == 1:
                print(f"¿Quisiste decir '{fixer[0]}'? s/n")
                ...
                # I decided to leave the user the possibility to save the word they wrote if they want it in the sentence, for flexibility 
                    ...
                    print(f"Quieres guardar tu palabra? s/n")
                ...
                print("¿Quisiste decir una de las siguientes opciones?:")
            ...
            print(f"Esa palabra no esta en el lexicón,")
            print(f"Aquí tienes unas opciones:")
            no_word = random.sample(list(lexicon), 3)
